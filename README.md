...
| **Lark 企业应用** | 读写 | 需要 | 需要飞书完整集成 |
| **Lark Webhook** | 只写 | 不需要 | 最简通知（1 分钟搞定） |
...
需要在[飞书开放平台](https://open.feishu.cn)创建应用 → 开通 `im:message` 权限 → 添加机器人到群聊 → 获取 chat_id。
...
project:
  name: my-backend
sync:
  # primary: local     # 不设则默认 git > local > http > lark > larkWebhook
  git:
    repo: https://github.com/your-org/synapse-hub.git
    branch: main
  local:
...
| Pull | 只从**主渠道**读取（默认优先级 git > local > http > lark > larkWebhook，可用 `primary` 覆盖） |
...
Synapse 的隔离边界是 **Git 仓库 / 本地目录 / HTTP 服务实例 / 飞书群**，不是 `project.name`。
...
产品 AI: synapse msg send "支持头像上传" --role pm --category requirement \
  --content "用户需要能上传头像..." --metadata '{"type":"requirement","priority":"high",...}'
